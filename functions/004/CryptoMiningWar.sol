...
interface MiniGameInterface {
    ...
    function isContractMiniGame()
        external
        pure
        returns (bool _isContractMiniGame);
}
...
contract CryptoMiningWar is PullPayment {
    ...
    event BuyMiner(
        address _addr,
        uint256[8] minerNumbers,
        uint256 _crystalsPrice,
        uint256 _hashrateBuy,
        uint256 _miningWarRound
    );
    event ChangeHasrate(
        address _addr,
    ...
    event ChangeCrystal(
        ...
        uint256 _type,
        uint256 _miningWarRound
    ); //_type: 1 add crystal , 2: sub crystal
    event BuyBooster(
        address _addr,
        uint256 _miningWarRound,
        uint256 _boosterId,
    ...
    function addMiner(
        ...
        uint256 idx,
        uint256 _value
    ) public isNotOver isCurrentRound(_addr) isAdministrator {
     ...
     */
    function addCrystal(address _addr, uint256 _value)
        public
        onlyContractsMiniGame
        isNotOver
        isCurrentRound(_addr)
    {
     ...
     * @param _addr player address
     */
    function subCrystal(address _addr, uint256 _value)
        public
        onlyContractsMiniGame
        isNotOver
        isCurrentRound(_addr)
     ...
     * msg.sender should be in the list of mini game
     */
    function addHashrate(address _addr, uint256 _value)
        public
        onlyContractsMiniGame
        isNotOver
        isCurrentRound(_addr)
     ...
     * msg.sender should be in the list of mini game
     */
    function subHashrate(address _addr, uint256 _value)
        public
        onlyContractsMiniGame
        isNotOver
        isCurrentRound(_addr)
    ...
    function setEngineerInterface(address _addr) public isAdministrator {
        CryptoEngineerInterface engineerInterface = CryptoEngineerInterface(
            _addr
        );
    ...
    function setupMiniGame() private {
        for (uint256 index = 0; index < totalMiniGame; index++) {
            if (miniGames[miniGameAddress[index]] == true) {
                MiniGameInterface MiniGame = MiniGameInterface(
                    miniGameAddress[index]
                );
    ...
    function lottery() public disableContract {
        ...
        for (uint256 idx = 0; idx < 10; idx++) {
            if (rankList[idx] != 0) {
                rankPayment = SafeMath.div(
                    SafeMath.mul(balance, profit[idx]),
                    100
                );
    ...
    function getFreeMiner(address _addr) public isNotOver disableContract {
        ...
        //reset player data
        if (p.hashrate > 0) {
            for (uint256 idx = 1; idx < numberOfMiners; idx++) {
                p.minerCount[idx] = 0;
            }
        }
    ...
    function getFreeMinerForMiniGame(address _addr)
        public
        isNotOver
        onlyContractsMiniGame
    {
    ...
    function buyMiner(uint256[8] minerNumbers)
        public
        isNotOver
        isCurrentRound(msg.sender)
    {
        ...
        for (uint256 minerIdx = 0; minerIdx < numberOfMiners; minerIdx++) {
            ...
            if (minerNumber > 0) {
                price = SafeMath.add(
                    price,
                    SafeMath.mul(m.basePrice, minerNumber)
                );

                uint256 currentMinerCount = p.minerCount[minerIdx];
                p.minerCount[minerIdx] = SafeMath.min(
                    m.limit,
                    SafeMath.add(p.minerCount[minerIdx], minerNumber)
                );
                // calculate no hashrate you want buy
                hashrate = SafeMath.add(
                    hashrate,
                    SafeMath.mul(
                        SafeMath.sub(p.minerCount[minerIdx], currentMinerCount),
                        m.baseProduct
                    )
                );
            }
        ...
        emit BuyMiner(
            msg.sender,
            minerNumbers,
            SafeMath.div(price, CRTSTAL_MINING_PERIOD),
            hashrate,
            roundNumber
        );
    }

    function getPlayerData(address addr)
        public
        view
        returns (
            uint256 crystals,
        ...
        )
    {
        ...
        crystals = SafeMath.div(
            calCurrentCrystals(_addr),
            CRTSTAL_MINING_PERIOD
        );
    ...
    function getHashratePerDay(address _addr)
        public
        view
        returns (uint256 personalProduction)
    {
        ...
        if (boosterIdx != 999) {
            ...
            personalProduction = SafeMath.div(
                SafeMath.mul(personalProduction, b.boostRate),
                100
            );
        }
    }
    ...
    // BOOSTER
    //--------------------------------------------------------------------------
    function buyBooster(uint256 idx)
        public
        payable
        isNotOver
        isCurrentRound(msg.sender)
    {
        ...
        if (beneficiary != 0) {
            refundPrize = SafeMath.div(
                SafeMath.mul(getBoosterPrice(idx), 55),
                100
            );
            asyncSend(beneficiary, refundPrize);
            boosterReward[beneficiary] = SafeMath.add(
                boosterReward[beneficiary],
                refundPrize
            );
        }
        prizePool = SafeMath.add(
            prizePool,
            SafeMath.sub(msg.value, SafeMath.add(devFeePrize, refundPrize))
        );
        updateCrystal(msg.sender);
        updateCrystal(beneficiary);
        uint256 level = getCurrentLevel(
            b.startingLevel,
            b.startingTime,
            b.halfLife
        );
    ...
    function getBoosterData(uint256 idx)
        public
        view
        returns (
            address owner,
    ...
    function getBoosterPrice(uint256 index) public view returns (uint256) {
        ...
        return
            getCurrentPrice(
                getCurrentLevel(
                    booster.startingLevel,
                    booster.startingTime,
                    booster.halfLife
                )
            );
    }
     ...
     * @param _addr player address
     */
    function calCurrentCrystals(address _addr)
        public
        view
        returns (uint256 _currentCrystals)
    {
        Player memory p = players[_addr];
        ...
        if (hashratePerDay > 0)
            _currentCrystals = SafeMath.add(
                p.crystals,
                SafeMath.mul(hashratePerDay, secondsPassed)
            );
    }
    ...
    function getCurrentPrice(uint256 currentLevel)
        private
        view
        returns (uint256)
    {
    ...
    function quickSort(
        ...
        Player storage p = players[addr];
        while (i <= j) {
            while (players[list[uint256(i)]].hashrate > p.hashrate) i++;
            while (p.hashrate > players[list[uint256(j)]].hashrate) j--;
            if (i <= j) {
