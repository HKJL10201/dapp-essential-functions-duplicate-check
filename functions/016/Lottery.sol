...
contract Lottery {
    ...
    //contructor function
    function Lottery(uint256 _winningGuess) {
    ...
    function userTokens(address _user) public view returns (uint256) {
        return users[_user].tokensBought;
    }

    // returns the guess made by user so far
    function userGuesses(address _user) public view returns (uint256[]) {
    ...
    function addTokens() payable {
        uint256 present = 0;
        uint256 tokensToAdd = msg.value / (10**18);
    ...
    // to add user guesses
    function makeGuess(uint256 _userGuess) {
    ...
    function winnerAddress() returns (address) {
        for (uint256 i = 0; i < userAddresses.length; i++) {
            ...
            for (uint256 j = 0; j < user.guess.length; j++) {
    ...
    function getPrice() returns (uint256) {
        ...
        if (winner == owner) {
            ...
            uint256 toTransfer = address(this).balance / 2;
