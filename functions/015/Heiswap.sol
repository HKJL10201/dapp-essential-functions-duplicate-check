...
contract Heiswap {
    ...
    address payable public relayerAddress =
        0x20a4b066fc4F70b0245B43e2F5a781C6d1030748;
    ...
    function deposit(uint256[2] memory publicKey) public payable {
        ...
        if (etherDecimalVal > 1) {
            uint256 refundEtherDecimalVal = (etherDecimalVal - 1) *
                (1 ether / 10);
        ...
        for (uint8 i = 0; i < ring.dParticipantsNo; i++) {
            if (
                ring.publicKeys[i][0] == publicKey[0] &&
    ...
    function withdraw(
        ...
        for (i = 0; i < ring.wParticipantsNo; i++) {
            if (
                ring.keyImages[uint8(i)][0] == keyImage[0] &&
    ...
    function createRingHash(uint256 amountEther, uint256 index)
        internal
        view
        returns (bytes32)
    {
    ...
    function getRingHash(uint256 amountEther, uint256 index)
        public
        view
    ...
    // BigNum correctly....
    function getPublicKeys(uint256 amountEther, uint256 index)
        public
        view
        returns (bytes32[2][ringMaxParticipants] memory)
    {
        ...
        for (uint256 i = 0; i < ringMaxParticipants; i++) {
            publicKeys[i][0] = bytes32(
                rings[receivedEther][index].publicKeys[i][0]
            );
            publicKeys[i][1] = bytes32(
                rings[receivedEther][index].publicKeys[i][1]
            );
        }
    ...
    function getParticipants(uint256 amountEther, uint256 index)
        public
        view
        returns (uint8, uint8)
    {
    ...
    function getRingMaxParticipants() public pure returns (uint256) {
    ...
    function getCurrentRingIdx(uint256 amountEther)
        public
        view
        returns (uint256)
    {
    ...
    function floorEtherAndCheck(uint256 receivedAmount)
        internal
        view
        returns (uint256)
    {
